...
function Dispatcher(endpoint, advice, options) {
  ...
  this._discoverTransports()
    .catch(function(err) {
      debug('No transports available: %s', err && err.stack || err);
    });
}
...
Dispatcher.prototype = {
  ...
  _discoverTransports: function(allowedTransportTypes) {
    debug('_discoverTransports: %j', allowedTransportTypes);
    ...
    registeredTransports.forEach(function(transport) {
      ...
      try {
        var instance = new Klass(self, endpoint, this._advice);
    ...
    if (hasNewAsyncTransports && connectionPromises.length > 1) {
      ...
      Promise.all(connectionPromises.map(function(promise) {
    ...
    if (!this._disconnecting) {
      ...
      this._reselectTransport()
        .catch(function(err) {
           ...
           * before retrying
           */
          debug('Unable to reselect transport after down: %s', err && err.stack || err);
        })
        .done();
    }
  },
