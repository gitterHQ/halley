...
function Client(endpoint, options) {
  ...
  this.listenTo(this._state, 'error', function(e) {
    throw e;
  });
...
Client.prototype = {
  ...
  connect: function() {
    this._state.transitionIfPossible('reconnect');
  },
  ...
  _onEnterDisconnecting: function() {
    ...
    return this._sendMessage({
      ...
      .then(function(response) {
        ...
        this._state.transitionIfPossible(response.successful ?
          'disconnectSuccess' : 'disconnectFailure');
      });
  },
  ...
  subscribe: function(channel, onMessage, context) {
    ...
    var deferredSub = Subscription.createDeferred(this, channel, onMessage,
      context);
    ...
    this._state.waitFor({
      ...
      .then(function(response) {
        if (!response.successful) {
          debug('Subscription rejected for %s to %s', self._dispatcher.clientId,
