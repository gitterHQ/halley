...
function Client(endpoint, options) {
  ...
  // TODO: deal with transport events from the dispatcher
  // this.listenTo(this._dispatcher, 'transport:up', this._receiveMessage);
  // this.listenTo(this._dispatcher, 'transport:down', this._receiveMessage);
}
...
Client.prototype = {
  ...
  _handleAdvice: function(advice) {
    ...
    if (advice.timeout) _advice.timeout = advice.timeout;
    if (advice.reconnect) _advice.reconnect = advice.reconnect;
    if (advice.interval) _advice.interval = advice.interval;
  ...
  _sendConnect: function() {
    ...
    // Note: the dispatcher will add the connectionType once
    // it's been resolved
