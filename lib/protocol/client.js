...
function validateBayeuxResponse(response) {
  ...
  if (!response.successful) {
    var error = new BayeuxError(response.error);

    if (response.advice) {
      error.reconnect = response.advice.reconnect;
    }
...
function Client(endpoint, options) {
  ...
  this.listenTo(advice, 'advice:none', function() {
    return Promise.delay(0).bind(this).then(function() {
      return this.transitionState('disable', { dedup:true });
    });
  });
...
Client.prototype = {
  ...
  subscribe: function(channel, onMessage, context) {
    var subscribePromise = this._channels.subscribe(channel, onMessage, context);
  ...
  publish: function(channel, data, options) {
    ...
    return cancelBarrier(this.connect())
      .bind(this)
      .then(function() {
  ...
  /**
   * Transition error handler for HANDSHAKING state
   */
  _onErrorHANDSHAKING: function(e) {
    // If the connection should not be retried, throw the error
    // and do not attempt to rehandshake
    if (e.reconnect === 'none') {
      return;
    }
