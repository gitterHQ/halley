...
extend(WebSocketTransport.prototype, {
  ...
  _createConnectPromise: Promise.method(function() {
    ...
    if (!socket) {
      throw new TransportError('Sockets not supported');
    }
    ...
    return new Promise(function(resolve, reject, onCancel) {
      ...
      socket.onerror = function() {
        ...
        self.close();
        reject(new TransportError("Websocket connection failed before opening"));
      };

      socket.onclose = function(e) {
        debug('Websocket closed. code=%s reason=%s', e.code, e.reason);
        self.close();
        reject(new TransportError("Websocket connection failed: code=" + e.code + ": " + e.reason));
      };
