...
var StateMachineMixin = {
  ...
  resetTransition: Promise.method(function(transition, reason, options) {
    if (this._stateReset) return;

    this._stateReset = true;

    this._sequencer.clear(reason);
    this._pendingTransitions = {};
  ...
  _transitionError: function(err) {
    ...
    // Check if the state has an error handler...
    var handler = this['_onError' + state];
    if (handler) {
      var nextState = handler.call(this, err);
      if (nextState) {
        return this._dequeueTransition(nextState);
      }
      throw err;
    }
